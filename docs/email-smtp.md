...
Potato includes an **SMTP email sender** (stdlib `smtplib` — no extra deps).  
...
Source: `src/potato/accounts/email.py`
...
```python
from potato.accounts.email import build_verify_email, get_email_sender
...
```python
from potato.accounts.email import build_otp_email
...
```python
from potato.accounts.email import SmtpConfig, SmtpEmailSender, build_verify_email
