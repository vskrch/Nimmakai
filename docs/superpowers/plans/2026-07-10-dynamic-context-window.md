...
- [ ] Add `src/potato/catalog/context.py` with `extract_context_length(obj)`, `parse_context_from_text(text)`, `merge_context(existing, new)`
...
- [ ] `X-Potato-Context-Length` on routed chat responses when known
